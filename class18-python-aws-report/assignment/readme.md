...
1. **Main bucket**: `customer-bucket-307946636515`
   - Create a folder named `incoming/` inside this bucket

2. **Terraform state bucket**: `307946636515-terraform-state`
...
# Upload to S3
aws s3 cp test-file-2024-01-15.txt s3://customer-bucket-307946636515/incoming/
```
...
|----------|-------------|---------------|
| BUCKET_PATH | S3 bucket and path for incoming files | customer-bucket-307946636515/incoming/ |

### AWS Configuration

The project uses:
- Region: `us-east-1` (Mumbai)
- Terraform state: Stored in S3 bucket `307946636515-terraform-state`
